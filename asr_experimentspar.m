...
% =========================================================
%
% asr_experimentspar.m : Experiments using parallel computing
%
% see also asr_experiments.m for an implementation that does not use the 
% Toolbox of parallel computing.
%
...
% It executes 100 times algorithms 1 and 5 (asr and src) on the databases
% 2, 4 and 7 (yale, rr, lfwf) using default values but without stoplist
%
% options.NV=350;T = asr_experimentspar(1,1,50,options);
...
function T = asr_experimentspar(test_sets,test_prgs,N,options)
...
for i=1:C:N
    ...
    for j = 1:nt
        ...
        for k = 1:np
            ...
            spmd
               x = feval(prg,stbase,options);
            end
            for c=1:C
                acc(i-1+c,j,k) = x{c};
            end
            x = mean(acc(1:(i+C-1),j,k))*100;
            T(j,k) = x;
            fprintf('%3d) %10s/%10s = %6.2f%% \n',i+C-1,prg,stbase,mean(x(:)));
        end
    end
    fprintf('\n\nAverage with N=%d:\n',i+C-1)
    fprintf('             ');
